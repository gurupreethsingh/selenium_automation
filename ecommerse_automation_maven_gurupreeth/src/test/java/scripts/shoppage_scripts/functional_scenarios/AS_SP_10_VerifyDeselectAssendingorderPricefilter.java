...
public class AS_SP_10_VerifyDeselectAssendingorderPricefilter extends OpenClose {
...
//			System.out.println((i + 1) + " - " + eachProductName + " " + eachProductPrice);
		...
		// click on the accending order price button
		WebElement accendingOrderPriceButton = driver
				.findElement(By.cssSelector("div.flex.flex-wrap.gap-2 button:nth-of-type(2)"));
		Thread.sleep(500);
		accendingOrderPriceButton.click();
		Thread.sleep(2000);
...
//			System.out.println((i + 1) + " - " + eachProductName + " " + eachProductPrice);
		...
		WebElement defaultButton = driver.findElement(By.cssSelector("div.flex.flex-wrap.gap-2 button:nth-of-type(1)"));
		Thread.sleep(500);
		defaultButton.click();
		Thread.sleep(2000);

		List<WebElement> originalProducts = driver
				.findElements(By.cssSelector("main.w-full.flex-1 div.group div.mt-3>p.leading-snug"));
		totalProductCount = originalProducts.size();

		List<WebElement> originalallProductsSellingPrice = driver.findElements(By.cssSelector(
				"main.w-full.flex-1 div.group div.mt-3 div.mt-2.flex.items-baseline.gap-2>span:first-child"));
		System.out.println("Total product count : " + totalProductCount);

		System.out.println("After clicking on the default button");
		System.out.println("********************************************************");
		// print the product names and prices.
		for (int i = 0; i < totalProductCount; i++) {
			String eachProductName = originalProducts.get(i).getText();
			String eachProductPrice = originalallProductsSellingPrice.get(i).getText();
//			System.out.println((i + 1) + " - " + eachProductName + " " + eachProductPrice);
			System.out.println(eachProductPrice);
		}
		System.out.println("********************************************************");
